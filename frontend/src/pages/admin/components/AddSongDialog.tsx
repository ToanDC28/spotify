...
const AddSongDialog = () => {
    const { albums, message, addSong } = useMusicStore();
    const [open, setOpen] = useState(false);
    const [isLoading, setIsLoading] = useState(false);
    const [newSong, setNewSong] = useState<NewSongRequest>({
      title: '',
      artist: '',
      albumId: '',
      duration: ''
    ...
    const handleSubmit = async () => {
        ...
        try {
            if(files.image === null || files.audio === null) {
                return toast.error("Please upload an image and audio file");
            }
            ...
            toast.error("Fail to add song", error);
        }finally{
            setIsLoading(false);
        }
    }
